...
# Only apply print filtering in production, not during debugging
...
async def startup_event():
    ...
    for pid, printer in registry.printers.items():
        try:
            ...
            # Don't block startup, will retry in broadcast loop
        except Exception as e:
    ...
    async def status_broadcaster():
        ...
        while not shutdown_event.is_set():
            try:
                        ...
                        try:
                            ...
                            if hasattr(printer, "get_status"):
                                try:
                                    ...
                                    # Log timeout but don't crash the loop
                            ...
                            if hasattr(printer, "get_percentage"):
                                try:
                                    ...
                                    # Silently skip percentage on error
                                    pct_value = None
                            ...
                            if (now >= due or need_first_value) and hasattr(printer, "get_filament_info"):
                                try:
                                    ...
                                    # Silently skip filament info on timeout/error
                                    tray_type = None
                                finally:
                            ...
                            # Catch timeout at the per-printer level
                            print(f"⏱️ TIMEOUT for printer {pid} operations: {e}")
                        except Exception as e:
                            # Catch any other error at the per-printer level
                            print(f"❌ Error processing printer {pid}: {e}")
                await asyncio.sleep(1)
            except TimeoutError as e:
                # Catch any stray timeouts that escaped inner handlers
                ...
                await asyncio.sleep(1)
        print("✅ Status broadcaster stopped cleanly")
...
#Send G-code command
@app.post("/api/printers/{printer_id}/gcode")
async def send_gcode(printer_id: str, payload: dict):
    printer = registry.printers.get(printer_id)
    if not printer:
        raise HTTPException(404, "Printer not found")
    ...
    if not gcode:
        raise HTTPException(400, "Missing 'gcode' in request body")
    
    if hasattr(printer, "send_gcode"):
        result = await printer.send_gcode(gcode)
        return result
    raise HTTPException(400, "Printer does not support G-code commands")
